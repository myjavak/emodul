...
@st.cache_data(ttl=3600) # Cache token na 1 hodinu
...
@st.cache_data(ttl=65) # Cache status na 65 sekúnd (dlhšie ako 60s oneskorenie)
...
def log_temperature(status_data, log_file):
    ...
    for zone_name, reg_id in REGULATOR_IDS.items():
        ...
        if target_item:
            ...
            # Uložíme teplotu v stupňoch Celzia (raw / 10.0)
            log_entry[zone_name] = raw_current / 10.0 if isinstance(raw_current, (int, float)) else None

    # Vytvorenie DataFrame pre aktuálny záznam
    ...
    if os.path.exists(log_file):
        try:
            ...
            df_combined = new_df # Ak je súbor poškodený, začneme odznova s novými dátami
    else:
        df_combined = new_df
        
    # Odstránenie starých záznamov
...
def show_statistics_page(log_file, days_to_show):
    ...
    try:
        ...
        # Vykreslenie grafu
        # Streamlit automaticky použije rôzne farby pre stĺpce (zóny)
...
# --- HLAVNÁ APLIKÁCIA STREAMLIT (RUNNER) ---
try:
    ...
    # 3. Logovanie dát (Spustí sa pri každom načítaní stránky)
    ...
    if st.session_state.page == 'Control':
        ...
        # Zobrazenie Aktuálneho Stavu v 3 stĺpcoch
        cols = st.columns(3)
        ...
        # Používame aktuálnu nastavenú teplotu ako predvolenú pre vybranú zónu
